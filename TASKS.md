...
- [x] 创建项目级开发约束文档
- [ ] Task 1 页面和业务能力（现有代码作为基线，按下方重制计划重新验收）
- [ ] Task 2 页面和业务能力
- [ ] 真实行情接口或真实 AI 服务接入
...
- [ ] 启动后进入完整行情首页，底部导航可用且选中态明确。
- [ ] 行情、自选、AI 解读三个 Tab 均有可见内容和合理空状态。
- [ ] 行情列表至少展示 5 只股票，字段完整，支持滚动和星标操作。
- [ ] 点击股票整行进入详情，路由携带 `code` 或 `symbol`。
- [ ] 详情页展示价格、涨跌、最高、最低、成交量、成交额和可读走势。
- [ ] 详情页有独立 AI 区域，展示趋势、操作、风险、总结四类信息。
- [ ] 无效参数、空数据、加载中和错误状态均有可识别反馈。
- [ ] 首页和 AI 解读入口复用同一个 `stock_detail` 页面。
- [ ] 业务模型、Repository、页面和组件位于 `shared/src/commonMain`，不引入 Compose、Fragment 或 ViewModel。
- [ ] 视觉和交互验收优先于构建耗时；构建失败原因记录在进度文档，不伪装成完成。
